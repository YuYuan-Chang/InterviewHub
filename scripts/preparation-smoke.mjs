...
export async function preparationSmoke(api, check, alice, bob, collectionId) {
  ...
  check('cannot attach another user’s collection', foreignCollection.status === 404);
  ...
  check(
    ...
    (await request(`${plan}/tasks`, 'POST', { title: 'Intruder' }, bob.accessToken)).status === 404,
  );
  const tasks = [];
  for (const title of [
    'Review API design',
    ...
    tasks.push((await request(`${plan}/tasks`, 'POST', { title, dueDate: '2026-09-01' })).data);
  }
  const taskPath = `${plan}/tasks/${tasks[0].id}`;
  check(
    'completes a task explicitly',
    (await request(taskPath, 'PATCH', { completed: true })).data.completed === true,
  );
  await request(taskPath, 'PATCH', { completed: true });
  const page1 = await request(`${plan}/tasks?limit=1`);
  const page2 = await request(`${plan}/tasks?limit=1&cursor=${page1.data.nextCursor}`);
  check(
    'task pagination does not repeat rows',
    page1.data.items.length === 1 && page2.data.items[0].id !== page1.data.items[0].id,
  );
  ...
  check(
    ...
    (await request(taskPath, 'PATCH', { completed: false }, bob.accessToken)).status === 404,
  );
  check(
    'foreign task list returns 404',
    (await request(`${plan}/tasks`, 'GET', undefined, bob.accessToken)).status === 404,
  );
  check(
    'reopens a task',
    (await request(taskPath, 'PATCH', { completed: false, title: 'Review API contracts' })).data
      .completed === false,
  );
  check(
    'rejects invalid dates',
    (await request(taskPath, 'PATCH', { dueDate: '2026-02-30' })).status === 400,
  );
  check(
  ...
  check(
    ...
    (await request(interviewPath, 'PATCH', { scheduledAt: later })).data.scheduledAt === later,
  );
  check(
    'foreign interview edit returns 404',
    (await request(interviewPath, 'PATCH', { status: 'cancelled' }, bob.accessToken)).status ===
      404,
  );
  ...
  check(
    ...
    upcoming.data.items.length === 1 && upcoming.data.items[0].id === interview.data.id,
  );
  ...
  check(
    ...
    ordered.data.items[0].id === earlier.data.id && ordered.data.items[1].id === interview.data.id,
  );
  check(
    'summary updates when an earlier interview is added',
    (await request('/summary')).data.nextInterview.id === earlier.data.id,
  );
  await request(`${plan}/interviews/${earlier.data.id}`, 'PATCH', { status: 'completed' });
  check(
  ...
  check(
    ...
    (await request('/summary', 'GET', undefined, bob.accessToken)).data.planCount === 0,
  );
  check(
    'other user’s schedule excludes private interviews',
    (await request('/interviews', 'GET', undefined, bob.accessToken)).data.items.length === 0,
  );
  ...
  check(
    ...
    (await request(plan, 'PATCH', { name: 'Updated preparation' })).status === 200,
  );
  check(
    'deleted collection can be unlinked',
    (await request(plan, 'PATCH', { collectionId: null })).data.collectionId === null,
  );
  ...
  check(
    ...
    (await request(interviewPath, 'PATCH', { status: 'completed' })).status === 404,
  );
}
