...
describe('preparation validation', () => {
  it('normalizes a plan and keeps optional fields empty', () => {
    ...
    expect(planSchema.safeParse({ name: 'Prep', collectionId: 'invalid' }).success).toBe(false);
  });
  it('preserves omitted patch fields instead of applying create defaults', () => {
    expect(planSchema.partial().parse({ name: 'New' })).toEqual({ name: 'New' });
    expect(taskSchema.partial().parse({ completed: true })).toEqual({ completed: true });
  });
  it('validates real calendar dates without timezone conversion', () => {
    expect(taskSchema.parse({ title: 'Practice', dueDate: '2028-02-29' }).dueDate).toBe(
      '2028-02-29',
    );
    for (const dueDate of ['2027-02-29', '2026-04-31', '2026-13-01', 'tomorrow']) {
      expect(taskSchema.safeParse({ title: 'Practice', dueDate }).success).toBe(false);
    }
  });
  it('requires explicit booleans and known interview stages/statuses', () => {
    expect(taskSchema.safeParse({ title: 'Task', completed: 'false' }).success).toBe(false);
    const valid = { stage: 'technical', scheduledAt: '2026-09-10T09:00:00+08:00' };
    expect(interviewSchema.parse(valid).status).toBe('scheduled');
    expect(interviewSchema.safeParse({ ...valid, stage: 'unknown' }).success).toBe(false);
    expect(interviewSchema.safeParse({ ...valid, scheduledAt: '2026-09-10T09:00' }).success).toBe(
      false,
    );
  });
});
...
describe('preparation pagination and schedule', () => {
  it('uses the id to break equal timestamp ties and rejects incompatible cursors', () => {
    ...
    expect(cursorWhere(cursor, 'createdAt')).toEqual({
      OR: [{ createdAt: { gt: new Date(time) } }, { createdAt: new Date(time), id: { gt: id } }],
    });
    expect(() => cursorWhere(cursor, 'scheduledAt')).toThrow('Malformed cursor');
    expect(() => cursorWhere(encodeCursor({ id: 123 }), 'createdAt')).toThrow('Malformed cursor');
