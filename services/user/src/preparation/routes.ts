...
async function owned(ownerId: string, id: string) {
  const plan = await prisma.preparationPlan.findFirst({ where: { id, ownerId } });
...
async function collectionOwner(ownerId: string, collectionId?: string | null) {
  ...
  const collection = await posts.get<{ ownerId: string }>(`/internal/collections/${collectionId}`);
  if (collection.ownerId !== ownerId) throw new HttpError(404, 'Collection not found');
}
...
function page<T extends { id: string }>(
  ...
  const last = items.at(-1) as (T & { createdAt?: Date; scheduledAt?: Date }) | undefined;
  return {
    items,
    nextCursor:
...
preparationRouter.get(`${base}/summary`, async (req, res) => {
  ...
  const [planCount, totalTasks, completedTasks, nextInterview] = await Promise.all([
    prisma.preparationPlan.count({ where: { ownerId } }),
    prisma.preparationTask.count({ where: { plan: { ownerId } } }),
    prisma.preparationTask.count({ where: { plan: { ownerId }, completed: true } }),
...
preparationRouter.get(`${base}/plans`, async (req, res) => {
  ...
  const rows = await prisma.preparationPlan.findMany({
    where: { ownerId: authedUser(req).id, ...cursorWhere(query.cursor, 'createdAt') },
...
preparationRouter.post(`${base}/plans`, validateBody(planSchema), async (req, res) => {
  const ownerId = authedUser(req).id;
  await collectionOwner(ownerId, req.body.collectionId);
  const plan = await prisma.preparationPlan.create({ data: { ...req.body, ownerId } });
...
preparationRouter.post(
  ...
  async (req, res) => {
    ...
    res
      .status(201)
      .json(await prisma.preparationTask.create({ data: { ...req.body, planId: plan.id } }));
  },
);
preparationRouter.patch(
  ...
  async (req, res) => {
    ...
    const result = await prisma.preparationTask.updateMany({ where, data: req.body });
    if (!result.count) throw new HttpError(404, 'Task not found');
    res.json(await prisma.preparationTask.findFirst({ where }));
  },
);
preparationRouter.delete(`${base}/plans/:planId/tasks/:taskId`, async (req, res) => {
    ...
    const rows = await prisma.preparationInterview.findMany({
      where: {
        ...
        AND: [
          ...
          query.view || !planId ? scheduleWhere(query.view ?? 'upcoming', new Date()) : {},
        ],
      },
      orderBy: [{ scheduledAt: 'asc' }, { id: 'asc' }],
      take: limit + 1,
  ...
  async (req, res) => {
    ...
    res.status(201).json(
      await prisma.preparationInterview.create({
        data: { ...req.body, scheduledAt: new Date(req.body.scheduledAt), planId: plan.id },
      }),
    );
  },
);
preparationRouter.patch(
  ...
  async (req, res) => {
    ...
    const result = await prisma.preparationInterview.updateMany({ where, data });
    if (!result.count) throw new HttpError(404, 'Interview not found');
    res.json(await prisma.preparationInterview.findFirst({ where }));
  },
);
preparationRouter.delete(`${base}/plans/:planId/interviews/:interviewId`, async (req, res) => {
